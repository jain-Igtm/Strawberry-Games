...
func _add_slide_access_stair(side_sign: float) -> void:
	...
	for i in range(step_count):
		...
		var tread_position := Vector3(x, top_y - 0.045, z)
		_mesh_box(tread_position, Vector3(stair_width, 0.09, run + 0.025), deck_tile)

		var riser_position := Vector3(x, top_y - rise * 0.5, z + run * 0.5)
	...
	var soffit_start := Vector3(x, 0.04, start_z + run * 0.35)
	var soffit_end := Vector3(
		x,
		3.20 - rise * 0.55,
	...
	var landing_center := Vector3(x, 3.16, -7.55)
	...
	for sign_value in [-1.0, 1.0]:
		...
		var rail_start := Vector3(x, rise + 0.92, start_z) + lateral
		var rail_end := Vector3(x, 3.20 + 0.92, start_z - float(step_count - 1) * run) + lateral
	...
	var landing_a := Vector3(landing_outer_x, 4.08, -8.55)
	var landing_b := Vector3(landing_outer_x, 4.08, -6.55)
