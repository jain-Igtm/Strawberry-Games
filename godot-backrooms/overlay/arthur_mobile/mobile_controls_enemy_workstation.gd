extends "res://arthur_mobile/mobile_controls.gd"

const LIGHT_ATTACK_SWIPE_PX := 78.0
const LIGHT_DIRECTION_DOMINANCE := 1.15
var light_attack_armed := false

func _input(event: InputEvent) -> void:
	if not OS.has_feature("mobile"):
		super._input(event)
		return

	if event is InputEventScreenTouch and event.pressed:
		if _inside_circle(event.position, _light_center(), ability_radius) and light_touch == -1:
			light_attack_armed = false

	# Own LIGHT drags completely on the enemy workstation so brightness cannot
	# prematurely claim a diagonal gesture that is still becoming an attack swipe.
	if event is InputEventScreenDrag and event.index == light_touch and light_touch != -1:
		light_drag_current = event.position
		var drag: Vector2 = light_drag_current - light_drag_origin

		if light_attack_armed:
			queue_redraw()
			return

		if not light_brightness_swipe:
			var upward_attack: bool = (
				drag.y <= -LIGHT_ATTACK_SWIPE_PX
				and absf(drag.y) > absf(drag.x) * LIGHT_DIRECTION_DOMINANCE
			)
			if upward_attack:
				# Upward launch wins even if the finger lingered long enough to begin
				# the normal spread hold. A player can therefore spread, then finish the
				# same gesture with an upward flick to launch those exact orb positions.
				light_attack_armed = true
				light_hold_active = false
				# Reuse the inherited gesture lock so the hold timer cannot reactivate.
				light_brightness_swipe = true
				light_tap_count = 0
				light_last_release_ms = -10000
				_ensure_lights_on()
				queue_redraw()
				return

		if not light_hold_active and not light_brightness_swipe:
			var horizontal_brightness: bool = (
				absf(drag.x) >= LIGHT_BRIGHTNESS_SWIPE_PX
				and absf(drag.x) > absf(drag.y) * LIGHT_DIRECTION_DOMINANCE
			)
			if horizontal_brightness:
				light_brightness_swipe = true
				light_tap_count = 0
				light_last_release_ms = -10000
				_ensure_lights_on()

		if light_brightness_swipe:
			if player != null and player.has_method("lights_adjust_brightness"):
				player.call("lights_adjust_brightness", event.relative.x / LIGHT_BRIGHTNESS_PIXELS_PER_UNIT)
			queue_redraw()
		return

	if event is InputEventScreenTouch and not event.pressed and event.index == light_touch and light_attack_armed:
		# Match normal hold-release semantics if the attack began after a spread:
		# launch from the current physical orb positions, then let the live formation
		# contract toward default while those same nodes fly back to it.
		if player != null and player.has_method("lights_return_radius"):
			player.call("lights_return_radius")
		if player != null and player.has_method("lights_end_radius_gesture"):
			player.call("lights_end_radius_gesture")
		if player != null and player.has_method("fire_psychic_light_attack"):
			player.call("fire_psychic_light_attack")
		light_touch = -1
		light_attack_armed = false
		light_hold_active = false
		light_brightness_swipe = false
		light_tap_count = 0
		light_last_release_ms = -10000
		queue_redraw()
		return

	super._input(event)
