...
func _update_center(force: bool) -> void:
	var next_center: Vector2i = Vector2i(roundi(player.global_position.x / CELL), roundi(player.global_position.z / CELL))
...
func _rebuild_queue() -> void:
	build_queue.clear()
	for ring: int in range(LOAD_RADIUS + 1):
...
func _add_cell(cell: Vector2i) -> void:
	...
	var scene: PackedScene = _scene_for_cell(cell)
	var architecture: Node = scene.instantiate()
	root.add_child(architecture)

	var light_roll: int = _cell_hash(cell.x, cell.y, 17)
	if light_roll % 13 == 0:
		var fixture: Node3D = LIGHT.instantiate() as Node3D
		fixture.rotation.y = float((light_roll >> 4) & 1) * PI * 0.5
		root.add_child(fixture)

	var prop_roll: int = _cell_hash(cell.x, cell.y, 97)
	if prop_roll % 109 == 0 and cell.distance_squared_to(Vector2i.ZERO) > 9:
		var crt: Node3D = CRT.instantiate() as Node3D
...
func _scene_for_cell(cell: Vector2i) -> PackedScene:
	...
	if abs(cell.x) <= 1 and abs(cell.y) <= 1:
		return NOWALL
	var roll: int = _cell_hash(cell.x, cell.y, 3) % 100
	if roll < 54:
		return NOWALL
	if roll < 66:
		return WALL_X
