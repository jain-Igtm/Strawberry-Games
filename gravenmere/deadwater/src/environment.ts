...
function addServiceBuilding(
  ...
  const shell = box(width, 4.8, depth, materials.darkRust, 0, 2.4, 0)
  shell.userData.blocksShot = true
  const roof = box(width + 0.7, 0.34, depth + 0.7, materials.blackMetal, 0, 4.94, 0)
  const foundation = box(width + 0.5, 0.34, depth + 0.5, materials.concrete, 0, 0.17, 0)
  const door = box(1.55, 2.5, 0.18, materials.blackMetal, 0, 1.35, -depth / 2 - 0.1)
  ...
  group.add(shell, roof, foundation, door, awning, vent, ventCap, sign)
  ...
  context.addCollider(x, z, swap ? depth : width, swap ? width : depth, 0.22)
}
