...
// A position-only lamp keeps illumination steady while the player looks
// around. Turning the camera no longer repaints walls with a moving cone.
const flashlight = new THREE.PointLight(0xf6fff2, 2.8, 20, 1.45);
...
const M = {
  ...
  // Floors use stable unlit colors so a camera light cannot project a giant
  // rectangular cone across them. Walls and objects still receive the lamp.
  floor: new THREE.MeshBasicMaterial({ color: 0x414741 }),
  floorLight: new THREE.MeshBasicMaterial({ color: 0x827f70 }),
...
function addDoor(group, x, floor, z, axis, room, direction = 1) {
  const doorWidth = 1.96;
  const doorHeight = 2.84;
  const halfWidth = doorWidth / 2;
  const hingeOffset = direction * halfWidth;

  // Put the pivot on the jamb, then offset the slab back to the doorway
  // center. Closed doors fill the frame; open doors swing from a real edge.
  const pivot = new THREE.Group();
  if (axis === 'x') pivot.position.set(x, floor, z + hingeOffset);
  else pivot.position.set(x + hingeOffset, floor, z);
  group.add(pivot);

  const slab = new THREE.Mesh(GEO, M.door);
  const hinge = new THREE.Mesh(GEO, M.metal);
  if (axis === 'x') {
    slab.scale.set(0.16, doorHeight, doorWidth);
    slab.position.set(0, doorHeight / 2, -hingeOffset);
    hinge.scale.set(0.2, doorHeight, 0.12);
  } else {
    slab.scale.set(doorWidth, doorHeight, 0.16);
    slab.position.set(-hingeOffset, doorHeight / 2, 0);
    hinge.scale.set(0.12, doorHeight, 0.2);
  }
  hinge.position.set(0, doorHeight / 2, 0);
  pivot.add(slab, hinge);

  const collider = {
    minX: x - (axis === 'x' ? 0.13 : halfWidth), maxX: x + (axis === 'x' ? 0.13 : halfWidth),
    minY: floor, maxY: floor + doorHeight,
    minZ: z - (axis === 'z' ? 0.13 : halfWidth), maxZ: z + (axis === 'z' ? 0.13 : halfWidth),
    enabled: true, owner: group
  };
  colliders.push(collider);
  group.userData.colliders.push(collider);
...
function colliderOverlapsPlayer(collider, x, z, floor, includeDisabled = false) {
  if (!collider.enabled && !includeDisabled) return false;
...
function use() {
  ...
  if (door.open && colliderOverlapsPlayer(door.collider, player.pos.x, player.pos.z, player.floor, true)) {
    showMessage('DOORWAY BLOCKED');
    return;
  }
  door.open = !door.open;
  const swing = door.axis === 'x' ? -door.direction : door.direction;
  door.pivot.rotation.y = door.open ? swing * Math.PI / 2 : 0;
...
function moveStick(touch) {
  ...
  const response = Math.min(1, amount / max * 1.45);
  joyX = dx / length * response;
  joyY = dy / length * response;
...
document.documentElement.dataset.samosborVersion = '24';
...
function frame(now) {
  ...
  // Preserve real walking speed during a slow mobile frame instead of
  // making low frame rate feel like movement through syrup.
