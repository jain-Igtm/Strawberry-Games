...
const camera = new THREE.PerspectiveCamera(72, innerWidth / innerHeight, 0.07, 245);
...
const player = {
  pos: new THREE.Vector3(0, 1.68, 0), yaw: Math.PI, pitch: 0,
...
function blockedAt(x, z, floor) {
  for (const collider of colliders) if (colliderOverlapsPlayer(collider, x, z, floor)) return true;
  return false;
}
...
function movePlayer(dx, dz) {
  ...
  for (let i = 0; i < steps; i++) {
    const nextFloorX = floorAt(player.pos.x + stepX, player.pos.z, player.floor);
    if (nextFloorX !== null && !blockedAt(player.pos.x + stepX, player.pos.z, nextFloorX)) {
      player.pos.x += stepX;
      player.floor = nextFloorX;
    }

    const nextFloorZ = floorAt(player.pos.x, player.pos.z + stepZ, player.floor);
    if (nextFloorZ !== null && !blockedAt(player.pos.x, player.pos.z + stepZ, nextFloorZ)) {
      player.pos.z += stepZ;
...
addEventListener('touchstart', (eventTouch) => {
  ...
  for (const touch of eventTouch.changedTouches) {
    ...
    if (touch.clientX < innerWidth * 0.44 && joyId === null) {
