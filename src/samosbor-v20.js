const response = await fetch('./samosbor.js?v=20', { cache: 'no-store' });
if (!response.ok) throw new Error(`Could not load Samosbor engine: ${response.status}`);
let source = await response.text();

const oldResize = `function resizeRenderer() {
  renderer.setPixelRatio(Math.min(devicePixelRatio, 1.15) * renderScale);
  renderer.setSize(innerWidth, innerHeight, false);
}`;
const newResize = `function resizeRenderer() {
  renderer.setPixelRatio(Math.min(devicePixelRatio, 1.15) * renderScale);
  // Keep performance scaling inside the drawing buffer. The canvas itself
  // must always cover the full phone viewport.
...
const replacements = [
  [oldResize, newResize, 'full-screen renderer sizing'],
...
for (const [before, after, label] of replacements) {
  if (!source.includes(before)) throw new Error(`Samosbor v20 patch could not find ${label}`);
